...
def dsonas_layer(data, num_filter, name, layer_num, bn_mom=0.9, workspace=256):
    ...
    for i in range(num_operation):
        if layer_num > 1:
            lambdas = mx.sym.Variable(shape=(1, input_num), dtype='float32', lr_mult=1., wd_mult=0,
                                      init=mx.init.One(), name="lambda_" + name + "_layer_%d_%d" % (layer_num, i + 1))
...
def dsonas_unit(data, num_filter, name, num_layers, id_conv=False, bn_mom=0.9, workspace=256):
    '''
    ...
    lambda_after = mx.sym.Variable(shape=(1, len(output)), dtype='float32', lr_mult=1.0, wd_mult=0,
                                   init=mx.init.One(), name="lambda_after_" + name)

    lambda_after = mx.sym.split(lambda_after, axis=1, num_outputs=len(output))
    for i in range(len(output)):
        output[i] = mx.sym.broadcast_mul(output[i], lambda_after[i])
...
def dsonas_reduction_unit(data, num_filter, name, bn_mom=0.9, workspace=256):
    lambdas = mx.sym.Variable(shape=(1, 2), dtype='float32', lr_mult=1., wd_mult=0,
...
def get_symbol_dsonas_search_full(num_classes, depth, image_shape, num_layers=2, conv_workspace=256, dtype='float32',
                                  **kwargs):
    """Return dsonas symbol for full search space
    ----------
    num_classes : int
