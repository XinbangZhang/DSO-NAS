    ...
    for i in range(num_stages):
        for j in range(units[i] - 1):
            body = dsonas_cell(body, filter_list[i + 1], name='stage%d_unit%d' % (i + 1, j + 1), num_layers=num_layers,
                               block_lambdas=block_lambdas, block_lambda_after=block_lambdas_after,
...
def get_symbol_dsonas_search_share(num_classes, depth, image_shape, num_layers=2, conv_workspace=256, dtype='float32',
                                   **kwargs):
