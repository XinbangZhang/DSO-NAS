...
def pruning_useless_lambda_share(load_param=None, num_stages=3, units=[], num_layers=2):
    ...
    for m in range(1, num_layers + 1):
        ...
        if m > 1:
            for n in range(1, num_operation + 1):
                load_param['lambda_stage%d_unit%d_layer_%d_%d' % (1, 1, m, n)] = \
                    load_param['lambda_layer_%d_%d' % (m, n)]
    pruning_useless_lambda_layer(load_param, 1, 1, num_layers=num_layers)
    ...
    for m in range(1, num_layers + 1):
        ...
        if m > 1:
            for n in range(1, num_operation + 1):
                load_param['lambda_layer_%d_%d' % (m, n)] = load_param[
                    'lambda_stage%d_unit%d_layer_%d_%d' % (1, 1, m, n)]

    return load_param


def dsonas_layer(data, num_filter, name, layer_num, load_param=None, block_lambda=None, bn_mom=0.9, workspace=256):
    input_num = len(data)
...
def dsonas_cell(data, num_filter, name, num_layers, block_lambdas=None, block_lambda_after=None,
                id_conv=False, load_param=None, bn_mom=0.9, workspace=256):
    '''
    ...
    for i in range(num_layers):
        layer = dsonas_layer(input, num_filter, name, layer_num=i + 1, load_param=load_param,
                             block_lambda=block_lambdas[i], bn_mom=bn_mom, workspace=workspace)
        num_outputs = len(layer)
        input += layer
        lambdas_after = load_param['lambda_after'].asnumpy().reshape(-1)
        lambdas_after = lambdas_after.tolist()
        for j in range(num_outputs):
            if lambdas_after[i * num_operation + j] > 0:
                assert layer[j] is not None
                output_temp.append(layer[j])
            else:
                output_temp.append(None)
    lambdas_after_split = mx.sym.split(block_lambda_after[0], axis=1, num_outputs=len(output_temp))
    output = []
    for i in range(len(output_temp)):
        if output_temp[i] is not None:
            output_temp[i] = mx.sym.broadcast_mul(output_temp[i], lambdas_after_split[i])
    ...
    output = mx.sym.BatchNorm(data=output, fix_gamma=False, eps=eps, momentum=bn_mom, name=name + '_bn')
    # menmonger
    if config.menmonger:
...
def dsonas_reduction_cell(data, num_filter, name, load_param=None, bn_mom=0.9, workspace=256):
    output = []
    lambdas = load_param["lambda_" + name].asnumpy().reshape(-1).tolist()
...
def dsonas_share_prune(units, num_stages, filter_list, num_classes, image_shape, num_layers=2, bn_mom=0.9,
                       workspace=256,
    ...
    for i in range(1, num_layers):
        ...
        for j in range(4):
            lambda_layer.append(
                mx.sym.Variable(shape=(1, 1 + i * 4), dtype='float32', lr_mult=1.0, wd_mult=0, init=mx.init.One(),
                                name="lambda_layer_%d_%d" % (i + 1, j + 1)))
        block_lambdas.append(lambda_layer)

    lambda_layer_after = mx.sym.Variable(shape=(1, num_layers * 4), dtype='float32', lr_mult=1.0,
                                         wd_mult=0, init=mx.init.One(), name="lambda_after")
    ...
    for i in range(num_stages):
        for j in range(units[i] - 1):
            body = dsonas_cell(body, filter_list[i + 1], name='stage%d_unit%d' % (i + 1, j + 1), num_layers=num_layers,
                               id_conv=(j == 0), load_param=load_param, block_lambdas=block_lambdas,
                               block_lambda_after=block_lambdas_after, bn_mom=bn_mom, workspace=workspace)
        if i != num_stages - 1:
...
def get_symbol_dsonas_search_share_prune(num_classes, depth, image_shape, num_layers=2, load_param=None,
                                         conv_workspace=256, dtype='float32', **kwargs):
    """Return dsonas symbol for share search space with useless connections pruned
    ----------
    num_classes : int
    ...
    load_param = pruning_useless_lambda_share(load_param, num_stages=num_stages, units=units, num_layers=num_layers)

    return dsonas_share_prune(units=units,
