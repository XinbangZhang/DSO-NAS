...
def dsonas_cell(data, num_filter, name, num_layers, block_lambdas=None, block_lambda_after=None, id_conv=False,
                load_param=None, bn_mom=0.9, workspace=256):
    ...
    for i in range(num_layers):
        ...
        input += layer
        if "lambda_after_" + name + "_layer_%d" % (i + 1) in load_param.keys():
...
def dsonas_share_prune(units, num_stages, filter_list, num_classes, image_shape, num_layers=2, bn_mom=0.9,
    ...
    lambda_layer_after = mx.sym.Variable(shape=(1, num_layers * num_operation), dtype='float32', lr_mult=1.0,
...
def get_symbol_dsonas_search_share_prune(num_classes, image_shape, num_layers=2, load_param=None, conv_workspace=256,
