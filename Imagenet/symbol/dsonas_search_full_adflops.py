...
def dsonas_layer(data, num_filter, name, layer_num, lambdas_split=None, bn_mom=0.9, workspace=256):
    input_num = len(data)
    scale_data = [[] for i in range(num_operation)]
    for i in range(num_operation):
        if layer_num > 1:
            for j in range(input_num):
                scale_data[i].append(mx.sym.broadcast_mul(data[j], lambdas_split[i * input_num + j]))
        else:
            scale_data[i] = data
...
def dsonas_cell(data, num_filter, name, num_layers, id_conv=False, bn_mom=0.9, workspace=256):
    ...
    lambda_num = num_layers * num_operation + sum(range(1, num_layers)) * num_operation * num_operation + \
                 len(range(1, num_layers)) * num_operation
    block_lambda = mx.sym.Variable(shape=(1, lambda_num), dtype='float32', lr_mult=1.0, wd_mult=0, init=mx.init.One(),
                                   name="lambda_" + name)
    ...
    for i in range(num_layers):
        layer = dsonas_layer(input, num_filter, name, layer_num=i + 1,
...
def dsonas_adflops(units, num_stages, filter_list, num_classes, image_shape, num_layers=2, bn_mom=0.9, workspace=256,
                   dtype='float32'):
    num_unit = len(units)
...
def get_symbol_dsonas_search_full_adflops(num_classes, image_shape, num_layers=2, conv_workspace=256, dtype='float32',
                                          **kwargs):
    """Return dsonas symbol for adoptive flops technique
    ----------
    num_classes : int
    ...
    return dsonas_adflops(units=units,
