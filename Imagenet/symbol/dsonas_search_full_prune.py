...
def pruning_useless_lambda_layer(load_param, stage, unit, num_layers=2):
    ...
    for m in range(1, num_layers + 1):
        lambda_after = load_param['lambda_after_stage%d_unit%d_layer_%d' % (stage, unit, m)].asnumpy().reshape(-1)
        for n in range(1, num_operation + 1):
            lambda_need['stage%d_unit%d_layer%d_op%d' % (stage, unit, m, n)] = lambda_after[n - 1]

    for m in range(num_layers, 1, -1):
        for mm in range(1, num_operation + 1):
            lambdas = load_param['lambda_stage%d_unit%d_layer_%d_%d' % (stage, unit, m, mm)].asnumpy().reshape(-1)
            if np.sum(lambdas) == 0:
                lambda_need['stage%d_unit%d_layer%d_op%d' % (stage, unit, m, mm)] = 0
            if lambda_need['stage%d_unit%d_layer%d_op%d' % (stage, unit, m, mm)] > 0:
                for n in range(1, lambdas.shape[0]):
                    lambda_need['stage%d_unit%d_layer%d_op%d' %
                                (stage, unit, int((n - 1) / num_operation) + 1, (n - 1) % num_operation + 1)] += \
                        lambdas[n]

    for m in range(1, num_layers + 1):
        for n in range(1, num_operation + 1):
            if m > 1:
                lambdas = load_param['lambda_stage%d_unit%d_layer_%d_%d' % (stage, unit, m, n)].asnumpy().reshape(-1)
                if np.sum(lambdas) == 0:
                    lambda_need['stage%d_unit%d_layer%d_op%d' % (stage, unit, m, n)] = 0

            if lambda_need['stage%d_unit%d_layer%d_op%d' % (stage, unit, m, n)] == 0:
                load_param['lambda_after_stage%d_unit%d_layer_%d' % (stage, unit, m)][0, n - 1] = 0
                for mm in range(m + 1, num_layers + 1):
                    for nn in range(1, num_operation + 1):
                        load_param['lambda_stage%d_unit%d_layer_%d_%d' % (stage, unit, mm, nn)][
                            0, (m - 1) * num_operation + n] = 0
                if m > 1:
                    load_param['lambda_stage%d_unit%d_layer_%d_%d' % (stage, unit, m, n)][:] = 0


def pruning_useless_lambda_full(load_param=None, num_stages=3, units=[], num_layers=2):
    for i in range(1, num_stages + 1):
        for j in range(1, units[i - 1]):
            for m in range(1, num_layers + 1):
                load_param['lambda_after_stage%d_unit%d_layer_%d' % (i, j, m)] = \
                    load_param['lambda_after_stage%d_unit%d' % (i, j)][:, (m - 1) * num_operation: m * num_operation]
    ...
    if layer_num == 1:
        ...
        for i in range(num_operation):
            ...
            lambdas_mult = mx.sym.Variable(name='lambda_' + name + '_layer_%d_%d' % (layer_num, i + 1), dtype='float32',
                                           lr_mult=1., wd_mult=0, shape=(1, input_num), init=mx.init.One())
...
def dsonas_reduction_cell(data, num_filter, name, load_param=None, bn_mom=0.9, workspace=256):
    ...
    lambdas_val = mx.sym.Variable(shape=(1, 2), dtype='float32', lr_mult=1., wd_mult=0,
...
def get_symbol_dsonas_search_full_prune(num_classes, image_shape, num_layers=2, load_param=None, conv_workspace=256,
