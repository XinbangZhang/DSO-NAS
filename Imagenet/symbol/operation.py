...
num_operation = 4
fix_gamma = False if config.retrain else config.fix_gamma
bn_wd = config.retrain_bn_wd
...
def Conv(data, num_filter=1, kernel=(1, 1), stride=(1, 1), pad=(0, 0), dilate=(1, 1), num_group=1, name='',
    ...
    bn = mx.sym.BatchNorm(data=conv, name='%s_bn' % (name), fix_gamma=fix_gamma, eps=2e-5, momentum=bn_mom)
...
def Pool(data, kernel=(1, 1), stride=(1, 1), pad=(1, 1), pool_type='max', name='', bn_mom=0.9):
    ...
    bn = mx.sym.BatchNorm(data=pool, name='%s_bn' % (name), fix_gamma=fix_gamma, eps=2e-5, momentum=bn_mom)
    act = mx.sym.Activation(data=bn, act_type='relu', name='%s_relu' % (name))
    return act
...
def aux_head(data, num_classes, bn_mom, dtype, grad_scale=0.4):
    ...
    data = mx.sym.Pooling(data=data, kernel=(5, 5), stride=(2, 2), pool_type='avg', name='aux_pool')
    ...
    if config.smooth_alpha:
        return mx.sym.SoftmaxOutput(data=fc1, grad_scale=grad_scale, smooth_alpha=0.1, name='aux_softmax')
    else:
