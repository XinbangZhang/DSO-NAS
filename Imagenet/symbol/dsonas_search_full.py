...
def dsonas_layer(data, num_filter, name, layer_num, bn_mom=0.9, workspace=256):
    ...
    for i in range(4):
        if layer_num > 1:
            lambdas = mx.sym.Variable(shape=(1, input_num), dtype='float32', lr_mult=1., wd_mult=0, init=mx.init.One(),
                                      name="lambda_" + name + "_layer_%d_%d" % (layer_num, i + 1))
...
def dsonas_cell(data, num_filter, name, num_layers, id_conv=False, bn_mom=0.9, workspace=256):
    ...
    lambda_after = mx.sym.Variable(shape=(1, len(output)), dtype='float32', lr_mult=1.0, wd_mult=0, init=mx.init.One(),
                                   name="lambda_after_" + name)
...
def dsonas_full(units, num_stages, filter_list, num_classes, image_shape, num_layers=2, bn_mom=0.9, workspace=256,
                dtype='float32'):
    num_unit = len(units)
...
def get_symbol_dsonas_search_full(num_classes, image_shape, num_layers=2, conv_workspace=256, dtype='float32',
                                  **kwargs):
    ...
    units = [3, 3, 14, 7]
    return dsonas_full(units=units,
