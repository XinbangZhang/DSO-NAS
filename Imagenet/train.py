...
from core.scheduler import liner_factor_scheduler
from core.scheduler import poly_factor_scheduler
...
def main(config):
    ...
    arg_params = None
    aux_params = None
    if config.retrain_model or config.load_model:
    ...
    if config.train_baseline:
        symbol_train = get_symbol_dsonas_baseline
    else:
        symbol_train = get_symbol_dsonas_evaluation_symbol

    symbol = symbol_train(num_classes=config.num_classes,
    ...
    if config.liner_lr_scheduler:
        lr_scheduler = liner_factor_scheduler(config.lr, config.lr_max, 5, config.num_epoch, epoch_size)
    elif config.poly_lr_scheduler:
        lr_scheduler = poly_factor_scheduler(config.lr, 240, epoch_size, lr_max=0.5)
    elif config.cos_lr_scheduler:
        lr_scheduler = cosine_factor_scheduler(0.0001, config.lr, 10, 2, epoch_size)
    elif config.step_lr_scheduler:
    ...
    shutil.copy('config/cfgs.py', os.path.join(model_dir, config.model_prefix + '.cfg'))
